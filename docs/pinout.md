# SensBlue Monarch — Pinout (HW rev 2.41)

All defines live in `src/SensBlueMonarchPins.h`.
...
| I²C SDA | IO21 | `SB_SDA` | expander, BME280, EEPROM, OLED |
| I²C SCL | IO22 | `SB_SCL` | idem |
| SPI MOSI | IO13 | `SB_SPI_MOSI` | piggyback |
| SPI MISO | IO12 | `SB_SPI_MISO` | piggyback. **Strapping pin** |
| SPI SCK  | IO14 | `SB_SPI_SCK` | piggyback |
| SPI SS   | IO5  | `SB_SPI_SS`  | piggyback. **Strapping pin** |
| UART1 TX (→ BG95 RXD_GSM) | IO25 | `SB_SERIAL1_TX` | via level shifter |
| UART1 RX (← BG95 TXD_GSM) | IO33 | `SB_SERIAL1_RX` | via level shifter |
| UART2 TX (mux)  | IO17 | `SB_SERIAL2_TX` | RS485 or GNSS |
| UART2 RX (mux)  | IO16 | `SB_SERIAL2_RX` | RS485 or GNSS |
...
| Bit | Define | Signal | Notes |
|---|---|---|---|
| P0_0 | `SB_EXP_GPS_EN` | GPS_EN | powers GNSS path |
...
| P0_4 | `SB_EXP_VBAT_2_EN` | VBAT_2_EN | biases VBAT divider |
| P0_5 | `SB_EXP_POWERKEY` | POWERKEY | BG95 PWRKEY (level shifted) |
| P0_6 | `SB_EXP_VCC_2_EN` | VCC_2_EN | secondary rail |
| P0_7 | — | — | unused (WP resistor removed on 2.41) |
| P1_0 | `SB_EXP_P1_0` | IOEXP_P1_0 | piggyback IO |
| P1_1 | `SB_EXP_P1_1` | IOEXP_P1_1 | piggyback IO |
| P1_2 | — | — | reserved |
| P1_3 | `SB_EXP_INPUT_IV_SEL` | INPUT_IV_SEL | 0 = 0-10 V, 1 = 4-20 mA |
| P1_4 | `SB_EXP_USART_SEL` | USART_SEL | 0 = RS485, 1 = GNSS |
...
wired to the ESP32. No interrupt-driven reads.

## I²C addresses (HW 2.41 defaults)

| Address | Define | Device |
|---|---|---|
| 0x20 | `SB_EXP_ADDR` | PCAL6416A expander |
| 0x50 | `SB_I2C_EEPROM_ADDR` | AT24C02 (256 bytes) |
| 0x77 | `SB_I2C_BME280_ADDR` | BME280 (SDO tied to VDD) |
| 0x3C | `SB_I2C_OLED_ADDR` | SSD1306 on header P2 |
